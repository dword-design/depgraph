...
export default async ({ isDuplicated } = {}) => {
  let modules =
    ...
    |> property('modules')
    |> filter(
      ({ source }) => multimatch(source, config.ignoreMatches).length === 0
    )
    |> map(module => ({
      ...
      label: module.coreModule
        ...
        : P.relative(process.cwd(), module.source),
  ...
  return isDuplicated
    ? (() => {
        const getContent = (currentModules, prefix = '') =>
          ...
          |> reduce((duplicatedModules, module) => {
            return [
              ...
              ...getContent(
                module.dependencies
                  |> map(({ target }) => modules |> find({ source: target })),
        ...
        const targetNames =
          ...
          |> uniq
        const sources =
          modules |> filter(({ source }) => !(targetNames |> includes(source)))
