...
export default {
  dot: () => withLocalTmpDir(async () => {
    ...
    const { all } = await execa.command('depgraph dot', { all: true })
  ...
  json: () => withLocalTmpDir(async () => {
    ...
    const { all } = await execa.command('depgraph json', { all: true })
    expect(all).toEqual(endent`
      [
        {
          "source": "src/foo.js",
